...
import { chunkToStreamEvent, printAgentOutput } from "./utils/print-agent-output.js";
...
async function* streamToAsyncIterable<T>(rs: ReadableStream<T>): AsyncIterable<T> {
...
if (!isDev) {
  ...
  app.get("/", (_req, res) => {
    res.json({ message: "Chat API server running. Use Vite dev server for frontend." });
  });
}
